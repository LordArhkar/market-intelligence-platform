"""
Backtest Validator V3 - Final improved strategy testing

Tests the V3 strategy with all improvements:
✅ No contradictory signals
✅ Trend alignment (price above/below 200 SMA)
...
Usage:
    python examples/backtest_validator_v3.py --symbol AAPL
    python examples/backtest_validator_v3.py --all
"""
...
class TechnicalAnalysis:
    ...
    @staticmethod
    def find_swing_low(candles, lookback=20):
        if len(candles) < lookback:
            return min(c.low for c in candles)
        return min(candles[-lookback:-1], key=lambda x: x.low).low
    
    @staticmethod
    def find_swing_high(candles, lookback=20):
        if len(candles) < lookback:
            return max(c.high for c in candles)
        return max(candles[-lookback:-1], key=lambda x: x.high).high
...
def generate_v3_signals(candles):
    """Generate signals using V3 strategy."""
    signals = []
    
    for i in range(250, len(candles)):  # Need 200 for SMA + patterns
        daily = candles[:i]
        current = daily[-1]
        ...
        has_bullish_pattern = any(k for k in patterns if "Bullish" in k or "Morning" in k)
        ...
        if daily_rsi > 70 and has_bullish_pattern:
            patterns = {k: v for k, v in patterns.items() if "Bullish" not in k and "Morning" not in k}
        ...
        # Trend bonus
        if above_200 and bullish_score > 0:
            bullish_score += 20
        elif not above_200 and bearish_score > 0:
            bearish_score += 20
        elif above_200 and bearish_score > 0:
            bearish_score -= 10
        elif not above_200 and bullish_score > 0:
            bullish_score -= 10
        ...
        if bullish_score > bearish_score * 1.3:
            ...
            stop = max(swing_low, current.close - atr * 2)
            ...
            stop = min(swing_high, current.close + atr * 2)
            target = current.close - atr * 2.5
        else:
            continue
...
def print_results(results, symbol=""):
    ...
    print(f"\n{'='*70}")
    print(f"📊 V3 BACKTEST RESULTS {f'for {symbol}' if symbol else ''}")
    print(f"{'='*70}")
    ...
    if len(results) >= 30:
        ...
        issues.append(f"⚠️  Only {len(results)} trades - need more data")
    
    wr = len(wins) / len(results) * 100
    if wr >= 50:
        successes.append(f"✅ Win rate ({wr:.1f}%) is good")
    elif wr >= 45:
        issues.append(f"⚠️  Win rate ({wr:.1f}%) is marginal")
    else:
...
def main():
    ...
    if args.all:
        symbols = ["AAPL", "MSFT", "GOOGL", "AMZN", "BTC-USD"]
    ...
    for symbol in symbols:
        ...
        signals = generate_v3_signals(candles)
